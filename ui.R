...
ui <- fluidPage(
  ...
  navbarPage(
    ...
    tabPanel("City Data",
             sidebarLayout(
               ...
               mainPanel(tabsetPanel(
                 tabPanel(
                   "Map",
                   tags$h3("Change in Median Sale Prices"),
                   leafletOutput("map"),
                   tags$div(
                     tags$br(),
                     tags$p(
                       "This map shows the location of the selected city/cities, indicated by a circle.
    ...
    tabPanel(
      "Predicted Prices for a City",
      sidebarLayout(sidebarPanel(
        selectInput(
          "city",
          label = "Select Your Desired City",
          choices = wa.cities$city,
          multiple = F,
          selected = "Seattle"
        )
